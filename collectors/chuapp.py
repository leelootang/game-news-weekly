"""
Fetch 触乐 (chuapp.com) articles from its standard RSS feed.

触乐 exposes a full-text WordPress RSS feed at https://www.chuapp.com/feed:
each <item> carries the canonical title, article URL, RFC-822 pubDate, author,
and a CDATA <description> holding the complete article body as HTML. That means
this collector needs no JavaScript, no browser and no per-article page fetch —
one HTTP GET yields every recent article with full text, so it runs on plain
urllib like steamdb_rankings rather than Playwright.

The window (--since/--until) is applied against each item's pubDate; only items
inside the window are stored. Records go to the flat root articles.jsonl via
write_article_record, keyed by the numeric article id parsed from the URL.
"""
...
FEED_URL = "https://www.chuapp.com/feed"
SOURCE_DOMAIN = "chuapp.com"
SOURCE_KEY = "chuapp"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch 触乐 articles from its RSS feed.")
