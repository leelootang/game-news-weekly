"""
Fetch 七麦研究院 (Qimai Research) articles via its official Sohu-号 (搜狐号) mirror.

七麦研究院's own site (qimai.cn/research) is a JS/login-gated shell that plain HTTP can't
read. But the account also publishes the same posts on its Sohu 自媒体 account, which
exposes a clean, urllib-friendly JSON API:

  https://v2.sohu.com/author-page-api/author-articles/pc/<authorId>?pageNumber=N&pageSize=20

authorId=100065715. Each item carries id/title/brief/link/publicTime; the article body is
read from the static page https://www.sohu.com/a/<id>_<authorId>. This mirrors the approach
used by youxiputao_sohu but is pure urllib (no Playwright needed for this account).

This demonstrates the "find the account on another channel" tactic: WeChat-only /
JS-gated accounts often have a scrapable Sohu-号 / B站 / 知乎 presence carrying the same content.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch 七麦研究院 articles via its Sohu-号 mirror.")
