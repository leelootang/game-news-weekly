"""
Fetch Deconstructor of Fun Deconstructions posts from Squarespace RSS and export PDFs.

The category URL supports format=rss and includes full content:encoded HTML.
Daily windows with zero matching posts are treated as normal successful runs.
"""
...
RSS_URL = "https://www.deconstructoroffun.com/blog?category=Deconstructions&format=rss"
SOURCE_DOMAIN = "deconstructoroffun.com"
SOURCE_KEY = "deconstructor_deconstructions"
...
def fetch_text(url: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt < 3:
                print(f"[rss] retry {attempt}/3 after Deconstructor of Fun RSS error: {exc}", file=sys.stderr)
                time.sleep(1.5 * attempt)
    raise RuntimeError(f"failed to fetch Deconstructor of Fun RSS: {url}: {last_exc}") from last_exc
...
def clean_content_html(content_html: str) -> str:
    ...
    content_html = re.sub(r"\s(?:srcset|sizes|loading|fetchpriority|decoding)=([\"']).*?\1", "", content_html, flags=re.I | re.S)
...
def collect_feed_items(since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    if max_pages != 1:
        print("[rss] Deconstructor of Fun category RSS is a single feed; --max-pages is accepted for runner compatibility")
...
def build_printable_html(item: NewsItem) -> str:
    ...
    categories = html.escape(", ".join(item.categories[:8]))
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">Deconstructor of Fun {author} {categories} {published}</div>
...
async def save_article_pdf(context, item: NewsItem, out_dir: Path, manifest: dict) -> bool:
    if not save_pdf_enabled():
        write_article_record(out_dir, manifest, item.news_id, {
            ...
            "extra": {"categories": item.categories, "image_url": item.image_url, "raw_published_at": item.raw_published_at},
        })
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch Deconstructor of Fun Deconstructions posts as PDFs.")
