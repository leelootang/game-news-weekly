"""
Fetch 旗舰评论 / Necromanov (necromanov.wordpress.com) articles from its RSS feed.

Necromanov (ロシツキー / 旗舰评论の作者) posts long-form game analysis on a
standard WordPress.com blog, which exposes a full-text RSS feed at
https://necromanov.wordpress.com/feed/. Each <item> carries the canonical title,
permalink, RFC-822 pubDate, dc:creator author and — crucially — the complete
article body inside <content:encoded> (WordPress full-text feeds put the whole
post there, while <description> holds only an excerpt). So one HTTP GET yields
every recent post with full text: no browser, no per-article fetch, plain urllib.

This is a very-low-frequency source (roughly one substantive post per year), so
an empty window is the normal case and never an error. The window (--since/--until)
is applied against each item's pubDate; records go to the flat root articles.jsonl
via write_article_record, keyed by the WordPress post id (?p=N) or a URL slug.
"""
...
FEED_URL = "https://necromanov.wordpress.com/feed/"
SOURCE_DOMAIN = "necromanov.wordpress.com"
SOURCE_KEY = "necromanov"
...
def make_item_id(url: str, guid: str) -> str:
    ...
    slug = re.search(r"necromanov\.wordpress\.com/(.+?)/?$", url)
    if slug:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch 旗舰评论/Necromanov articles from its WordPress RSS feed.")
