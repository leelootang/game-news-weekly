"""
Fetch 游戏葡萄 articles from its public Sohu media mirror and export PDFs.

This is a fallback discovery source for WeChat articles when Sogou Weixin does
not expose the account's recent timeline. The articles are mirrored from 游戏葡萄
on Sohu, so manifests identify the source as m.sohu.com rather than WeChat.
"""
...
LIST_URL = "https://m.sohu.com/media/204824"
SOURCE_DOMAIN = "m.sohu.com"
SOURCE_KEY = "youxiputao_sohu"
...
def make_news_id(url: str) -> str:
    match = re.search(r"/a/(\d+)_204824", url)
    if match:
        return match.group(1)
    return re.sub(r"\W+", "_", urlparse(url).path).strip("_") or "unknown"


def parse_list_time(value: str, now: datetime) -> datetime | None:
    ...
    if match:
        ...
        return today - timedelta(days=1) + timedelta(hours=hour, minutes=minute)
    match = re.search(r"前天\s*(\d{1,2}):(\d{2})", text)
    if match:
        hour, minute = map(int, match.groups())
        return today - timedelta(days=2) + timedelta(hours=hour, minutes=minute)
...
def parse_article_datetime(value: str) -> datetime | None:
    match = re.search(r"\d{4}-\d{1,2}-\d{1,2}\s+\d{1,2}:\d{2}", value.strip())
    if not match:
        return None
...
async def collect_news_items(page, since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    ...
    for row in rows:
        ...
        published_at = parse_list_time(raw_time, now)
    ...
    if not items and not older_count:
        print("[list] no in-window entries found on first timeline page", file=sys.stderr)
    ...
    return await page.evaluate(
        """() => {
            ...
            if (content) {
                ...
                content.querySelectorAll('script, iframe, .middle-mon-content, [class*="ad"]').forEach(el => el.remove());
            }
            return {
                title: text('h1') || text('.title'),
                published: text('.time') || text('[class*="time"]'),
                author: text('.author-name') || text('[class*="author"]'),
...
def build_printable_html(meta: dict[str, str]) -> str:
    title = html.escape(meta.get("title") or "untitled")
    author = html.escape(meta.get("author") or "游戏葡萄")
    published = html.escape(meta.get("published") or "")
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">{author} {published}</div>
...
async def save_article_pdf(context, item: NewsItem, out_dir: Path, manifest: dict) -> bool:
    ...
    try:
        ...
        published_at = parse_article_datetime(meta.get("published") or "") or item.published_at
        if not save_pdf_enabled():
            write_article_record(
                ...
                item.news_id,
                {
                    ...
                    "title": title,
                    "url": item.url,
                    "author": meta.get("author") or "游戏葡萄",
        ...
        await page.set_content(build_printable_html(meta), wait_until="domcontentloaded")
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch 游戏葡萄 Sohu mirror articles and save them as PDFs.")
    ...
    parser.add_argument("--max-pages", type=int, default=3, help="Timeline scroll depth")
