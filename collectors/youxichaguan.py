"""
Fetch 游戏茶馆 articles from the public WordPress API and export PDFs.

The site exposes posts through WordPress REST endpoints. This collector uses the
API as the discovery and content source, avoiding fragile homepage scraping.
"""
...
API_URL = "https://youxichaguan.com/wp-json/wp/v2/posts"
SOURCE_DOMAIN = "youxichaguan.com"
SOURCE_KEY = "youxichaguan"
...
def fetch_json(url: str) -> list[dict]:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt < 3:
                print(f"[api] retry {attempt}/3 after 游戏茶馆 API error: {exc}", file=sys.stderr)
                time.sleep(1.5 * attempt)
    raise RuntimeError(f"failed to fetch 游戏茶馆 API: {url}: {last_exc}") from last_exc
...
def collect_news_items(since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    ...
    for page_no in range(1, max_pages + 1):
        ...
        for post in posts:
            ...
            link = post.get("link") or f"https://youxichaguan.com/?p={news_id}"
            items.append(
                NewsItem(
                    news_id=news_id,
                    url=link,
                    title=title or f"youxichaguan-{news_id}",
    ...
    if not reached_older_boundary and max_pages > 0:
        raise RuntimeError(
            f"hit --max-pages={max_pages} before proving 游戏茶馆 moved older than requested window"
        )
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">游戏茶馆 {published}</div>
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch 游戏茶馆 articles and save them as PDFs.")
