"""
Fetch MobileGamer.biz articles from the public WordPress RSS feed and export PDFs.
...
RSS_URL = "https://mobilegamer.biz/feed/"
SOURCE_DOMAIN = "mobilegamer.biz"
SOURCE_KEY = "mobilegamer"
...
def fetch_text(url: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt < 3:
                print(f"[rss] retry {attempt}/3 after MobileGamer.biz RSS error: {exc}", file=sys.stderr)
                time.sleep(1.5 * attempt)
    raise RuntimeError(f"failed to fetch MobileGamer.biz RSS: {url}: {last_exc}") from last_exc
...
def collect_feed_items(since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    if max_pages != 1:
        print("[rss] MobileGamer.biz RSS is a single feed; --max-pages is accepted for runner compatibility")
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">MobileGamer.biz {author} {categories} {published}</div>
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch MobileGamer.biz articles and save them as PDFs.")
