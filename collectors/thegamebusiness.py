...
SOURCE_DOMAIN = "thegamebusiness.com"
SOURCE_KEY = "thegamebusiness"
...
def fetch_text(url: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt < 3:
                print(f"[rss] retry {attempt}/3 after The Game Business RSS error: {exc}", file=sys.stderr)
                time.sleep(1.5 * attempt)
    raise RuntimeError(f"failed to fetch The Game Business RSS: {url}: {last_exc}") from last_exc
...
def collect_feed_items(since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    if max_pages != 1:
        print("[rss] The Game Business Substack RSS is a single feed; --max-pages is accepted for runner compatibility")
    ...
    for node in root.findall("./channel/item"):
        ...
        items.append(
            NewsItem(
                ...
                url=url,
                title=title,
                author=item_text(node, f"{DC_NS}creator") or "Christopher Dring",
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">The Game Business {author} {source} {published}</div>
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch The Game Business posts and save them as PDFs.")
