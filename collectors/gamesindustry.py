"""
Fetch GamesIndustry.biz articles from the public RSS feed and export PDFs.

The feed contains stable article URLs, titles, authors, categories, and UTC
publication timestamps. Detail pages are opened only for full article body
extraction before rendering a clean local PDF.
"""
...
RSS_URL = "https://www.gamesindustry.biz/feed"
SOURCE_DOMAIN = "gamesindustry.biz"
SOURCE_KEY = "gamesindustry"
...
def fetch_text(url: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt < 3:
                print(f"[rss] retry {attempt}/3 after GamesIndustry.biz RSS error: {exc}", file=sys.stderr)
                time.sleep(1.5 * attempt)
    raise RuntimeError(f"failed to fetch GamesIndustry.biz RSS: {url}: {last_exc}") from last_exc
...
def media_image_url(item: ET.Element) -> str:
    for node in item.findall("{http://search.yahoo.com/mrss/}content"):
        ...
        if url and (medium == "image" or re.search(r"\.(jpe?g|png|webp)(?:\?|$)", url, re.I)):
            return url
    match = re.search(r'<img[^>]+src=["\']([^"\']+)["\']', item_text(item, "description"), re.I)
...
def collect_feed_items(since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    if max_pages != 1:
        print("[rss] GamesIndustry.biz RSS is a single feed; --max-pages is accepted for runner compatibility")
...
async def fetch_article_content(context, item: NewsItem) -> NewsItem:
    ...
    try:
        ...
        try:
            await page.wait_for_selector(".article_body_content, article", timeout=15_000)
        ...
        meta = await page.evaluate(
            """() => {
                ...
                const body = document.querySelector('.article_body_content') || document.querySelector('article');
                if (body) {
                    ...
                    body.querySelectorAll('script, iframe, style, .inlinead, .advert, .newsletter, .social, .share').forEach(el => el.remove());
                }
                return {
                    title,
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">GamesIndustry.biz {author} {categories} {published}</div>
    {hero}
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch GamesIndustry.biz articles and save them as PDFs.")
