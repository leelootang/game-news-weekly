"""
Fetch 宝玉的分享 (baoyu.io) blog posts — LLM / prompt-engineering / software.

baoyu.io publishes an RSS feed at https://baoyu.io/feed.xml whose <item>s carry
the canonical title, article URL and an RFC-822 pubDate, but only a short
summary in <description>. The article pages themselves are server-rendered:
the full post body lives in a `<div class="prose …">` block. So this collector
reads the feed for the recent post list, then opens each in-window article page
to pull the full text out of its prose container. Plain urllib + regex.
"""
...
def make_item_id(url: str) -> str:
    m = re.search(r"/(?:blog|translations)/(.+?)/?$", url)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch 宝玉的分享 blog posts.")
    ...
    for e in in_window:
        ...
        if not page:
            continue
        body_html = extract_balanced_div(page, "prose")
        text = html_to_text(body_html)
        if not text:
            continue
        item_id = make_item_id(e["url"])
        ...
        ok += 1
        print(f"[{item_id}] saved  {e['published_at']:%Y-%m-%d}  {e['title'][:40]}")
