...
def media_image_url(item: ET.Element) -> str:
    ...
    if enclosure is not None:
        url = enclosure.attrib.get("url") or ""
        if url:
            return url
    ...
    try:
        ...
        meta = await page.evaluate(
            """() => {
                ...
                const strap = text('article .strap');
                const authorText = text('article .byline');
                const body = document.querySelector('article .content .main .body') || document.querySelector('article .body');
                if (body) {
                    ...
                    body.querySelectorAll('script, iframe, style, .art-subscribe, .inlinead, .advert, .newsletter, .social, .share-area').forEach(el => el.remove());
                }
                return {
                    title,
                    strap,
        ...
        strap = html.escape(meta.get("strap") or "")
        if strap:
            content_html = f'<p class="strap">{strap}</p>\n{content_html}'
        return NewsItem(
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">PocketGamer.biz {author} {categories} {published}</div>
    {hero}
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch PocketGamer.biz articles and save them as PDFs.")
