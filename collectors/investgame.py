"""
Fetch InvestGame.net articles from the public WordPress RSS feed and export PDFs.

The feed exposes article URLs, titles, authors, categories, publication
timestamps, and content:encoded HTML. Daily windows with zero matching articles
are treated as normal successful runs.
"""
...
RSS_URL = "https://investgame.net/feed/"
SOURCE_DOMAIN = "investgame.net"
SOURCE_KEY = "investgame"
...
def fetch_text(url: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt < 3:
                print(f"[rss] retry {attempt}/3 after InvestGame.net RSS error: {exc}", file=sys.stderr)
                time.sleep(1.5 * attempt)
    raise RuntimeError(f"failed to fetch InvestGame.net RSS: {url}: {last_exc}") from last_exc
...
def clean_content_html(content_html: str) -> str:
    ...
    content_html = re.sub(r"<p>\s*Views:\s*\d+\s*</p>", "", content_html, flags=re.I)
...
def collect_feed_items(since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    if max_pages != 1:
        print("[rss] InvestGame.net RSS is a single feed; --max-pages is accepted for runner compatibility")
    ...
    print(
        f"[rss] collected {len(items)} article(s), older={older}, "
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">InvestGame.net {author} {categories} {published}</div>
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch InvestGame.net articles and save them as PDFs.")
