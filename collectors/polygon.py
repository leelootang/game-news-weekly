"""
Fetch Polygon (polygon.com) articles from its public RSS feed.

Polygon exposes a standard RSS feed at https://www.polygon.com/rss/index.xml.
Each <item> carries the canonical title, permalink, RFC-822 pubDate, author and the
full article body inside <content:encoded>. So one HTTP GET yields recent posts with
full text — no browser, no per-article fetch, plain urllib. (The HTML site is fine too,
but the RSS is the cleanest, lowest-noise entry.)

Polygon covers games + broader entertainment; downstream 6-dim tagging + editorial
curation handle topical filtering, so this collector ingests the feed as-is.
Records go to the flat root articles.jsonl via write_article_record, keyed by the
numeric story id in the URL (falling back to a slug).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch Polygon articles from its RSS feed.")
