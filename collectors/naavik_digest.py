"""
Fetch Naavik Digest newsletter posts from the category RSS feed and export PDFs.

Naavik is WordPress-backed and exposes full Digest posts at
/category/digest/feed/. The collector treats zero posts in a daily window as a
...
RSS_URL = "https://naavik.co/category/digest/feed/"
SOURCE_DOMAIN = "naavik.co"
SOURCE_KEY = "naavik_digest"
...
def fetch_text(url: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt < 3:
                print(f"[rss] retry {attempt}/3 after Naavik Digest RSS error: {exc}", file=sys.stderr)
                time.sleep(1.5 * attempt)
    raise RuntimeError(f"failed to fetch Naavik Digest RSS: {url}: {last_exc}") from last_exc
...
def collect_feed_items(since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    if max_pages != 1:
        print("[rss] Naavik Digest RSS is a single feed; --max-pages is accepted for runner compatibility")
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">Naavik Digest {author} {categories} {published}</div>
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch Naavik Digest posts and save them as PDFs.")
