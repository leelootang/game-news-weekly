"""
Fetch The GameDiscoverCo newsletter posts from Substack RSS and export PDFs.

Substack RSS is used only to discover posts and publication dates.  Its
``content:encoded`` field can be a teaser even when the public article page is
complete, so every in-window item is fetched from its canonical article page
before being stored.
"""
...
RSS_URL = "https://newsletter.gamediscover.co/feed"
SOURCE_DOMAIN = "newsletter.gamediscover.co"
SOURCE_KEY = "gamediscover"
...
def fetch_text(url: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt < 3:
                print(f"[rss] retry {attempt}/3 after GameDiscoverCo RSS error: {exc}", file=sys.stderr)
                time.sleep(1.5 * attempt)
    raise RuntimeError(f"failed to fetch GameDiscoverCo RSS: {url}: {last_exc}") from last_exc
...
def first_image_url(content_html: str, description_html: str) -> str:
    for source in (content_html, description_html):
        match = re.search(r'<img[^>]+src=["\']([^"\']+)["\']', source, re.I)
        if match:
...
def clean_content_html(content_html: str) -> str:
    ...
    content_html = re.sub(r"\s(?:srcset|sizes|loading|fetchpriority)=([\"']).*?\1", "", content_html, flags=re.I | re.S)
...
def collect_feed_items(since: datetime, until: datetime, max_pages: int) -> list[NewsItem]:
    if max_pages != 1:
        print("[rss] GameDiscoverCo Substack RSS is a single feed; --max-pages is accepted for runner compatibility")
    ...
    for node in root.findall("./channel/item"):
        ...
        url = item_text(node, "link") or item_text(node, "guid")
        ...
        description_html = clean_content_html(item_text(node, "description"))
        content_html = clean_content_html(item_text(node, f"{CONTENT_NS}encoded"))
        if not content_html:
        ...
        items.append(
            NewsItem(
                ...
                url=url,
                title=title,
                author=item_text(node, f"{DC_NS}creator") or "Simon Carless",
                description_html=description_html,
                image_url=first_image_url(content_html, description_html),
    ...
    print(
        f"[rss] collected {len(items)} post(s), older={older}, "
        f"future_or_current={skipped_future}, missing_date={missing_date}"
    )
    if not older:
        print("[rss] warning: feed did not include an item older than --since; using all in-window RSS items")
    items.sort(key=lambda x: x.published_at)
    return items


async def fetch_article_content(context, item: NewsItem) -> NewsItem:
    """Read the public post page; never treat the RSS teaser as article body."""
...
def build_printable_html(item: NewsItem) -> str:
    ...
    source = html.escape(SOURCE_DOMAIN)
...
<body>
  <main>
    <h1>{title}</h1>
    <div class="meta">The GameDiscoverCo newsletter {author} {source} {published}</div>
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch GameDiscoverCo newsletter posts and save them as PDFs.")
    ...
    print(f"[rss] selected {len(items)} post(s)")
